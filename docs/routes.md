...
| `/prevencion-en-parques-tierra-sombra` | `pages/prevencion-en-parques-tierra-sombra.astro` | `emergencyTopics` + `parkModules` | 1 | Parks menu, "tierra" palette (light ground). Renders `components/ParksMenu.astro` with `theme="tierra"`; **not linked from any page** (orphaned) — a style option under review, see the parks-menu PR |
| `/prevencion-en-parques-verde-contraste` | `pages/prevencion-en-parques-verde-contraste.astro` | `emergencyTopics` + `parkModules` | 1 | Same menu, "bosque" palette (dark ground, `mode="dark"` footer). Same component, `theme="bosque"`; **not linked from any page** (orphaned), same review |
...
**Total: 50 routes** (1+1+1+6+1+6+2+8+24). Reachability: `/`, the two menus, and their 12 `[topic]` children are reachable; the 8 `/[slug]-[mode]` routes are reachable from outside the set only via `/general` (the `-light` four by static `href`, the `-dark` four by its theme-swap script). **Orphaned** (no entry point from outside the set): `/general` itself — which nothing links — the 24 `/[slug]-[style]-[mode]` routes, whose only inbound `href`s come from within the set and, for the style switch, only in preview builds (see issue #7 on linking or removing them), and the 2 `/prevencion-en-parques-*` routes, which are deliberately unlinked style options awaiting a decision on which palette ships.
