...
| `/prevencion-en-parques-tierra-sombra` | `pages/prevencion-en-parques-tierra-sombra.astro` | `parkTopics` + `parkModules` | 1 | Parks menu carrying the researched, source-cited topic set. Renders `components/ParksMenu.astro` with `theme="tierra"` and `topicSet="parques"`; linked from `/` as "Menú 03" |
| `/prevencion-en-parques-verde-contraste` | `pages/prevencion-en-parques-verde-contraste.astro` | `emergencyTopics` + `parkModules` | 1 | Same menu and same `tierra` palette, kept as a comparison against the generic site-wide topic set. Same component, `theme="tierra"`, default `topicSet`; linked from `/` as "Menú 04". The route slug still says `verde-contraste` for continuity; the green palette itself is no longer used |
| `/prevencion-en-parques-tierra-sombra/[topic]` | `pages/prevencion-en-parques-tierra-sombra/[topic].astro` | `parkTopics` (`.id`) | 5 | The five researched park topics, rendered by `ParkTopicSection` inside `.pc-theme-tierra`. Each topic picks one of the three documented style variants via `styleVariant`, and each page ends with a cited `Fuentes` block; back button to the menu |
| `/prevencion-en-parques-verde-contraste/[topic]` | `pages/prevencion-en-parques-verde-contraste/[topic].astro` | `emergencyTopics` (`.id`) | 6 | The generic six topics, now also inside `.pc-theme-tierra`; back button to its own menu |
...
**Total: 61 routes** (1+1+1+6+1+6+2+5+6+8+24). Reachability: `/`, the two menus, and their 12 `[topic]` children are reachable; the 8 `/[slug]-[mode]` routes are reachable from outside the set only via `/general` (the `-light` four by static `href`, the `-dark` four by its theme-swap script). The 2 `/prevencion-en-parques-*` menus are reachable from `/` as "Menú 03" and "Menú 04"; the first reaches its 5 `[topic]` children and the second its 6. Those children exist so each menu carries its own content set without touching `/prevencion-y-gestion-de-riesgo/[topic]`, which is shared with the original menu and must stay blue. The palette question is settled — both menus now use `tierra`; what remains open is whether the generic topic set is retired in favour of the researched one, which would remove the `verde-contraste` menu page and its 6 children. **Orphaned** (no entry point from outside the set): `/general` itself — which nothing links — and the 24 `/[slug]-[style]-[mode]` routes, whose only inbound `href`s come from within the set and, for the style switch, only in preview builds — see issue #7 on linking or removing them.
